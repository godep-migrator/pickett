...
func LoadConfiguration(log *Logger, filename string) {
	log.Close(0)
	...
	for _, xmlfilt := range xc.Filter {
		...
		err := log.AddFilter(xmlfilt.Tag, lvl, nil, nil, filt)
		if err != nil {
			fmt.Fprintf(os.Stderr, "LoadConfiguration: Duplicate field name%s in %s\n", xmlfilt.Tag, filename)
			os.Exit(1)
		}
	}
}

func xmlToConsoleLogWriter(filename string, props []xmlProperty, enabled bool) (LogWriter, bool) {
	// Parse properties
...
func xmlToFileLogWriter(filename string, props []xmlProperty, enabled bool) (LogWriter, bool) {
	file := ""
	format := "[%D %T] [%L] (%S) %M"
	maxlines := 0
	maxsize := 0
...
func xmlToXMLLogWriter(filename string, props []xmlProperty, enabled bool) (LogWriter, bool) {
	file := ""
	maxrecords := 0
	maxsize := 0
	daily := false
...
func xmlToSocketLogWriter(filename string, props []xmlProperty, enabled bool) (LogWriter, bool) {
	endpoint := ""
	protocol := "udp"
